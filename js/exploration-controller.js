...
function crawlerState(){const node=currentNode(),grid=!shared?.transit?roomGrid(node):null,pos=grid?loadHero(node):null;return {node:shared?.transit?.from||shared?.node,facing:viewFacing,roomGrid:grid,heroPosition:pos,visibleFeatures:grid?crawlerMarkers(node,pos):[],sharedState:shared}}
function renderRoomExplore(){const node=currentNode(),grid=roomGrid(node),box=document.querySelector('#roomExplore');if(!box)return;const enabled=Boolean(grid&&!shared?.transit);box.hidden=!enabled;if(!enabled){const list=document.querySelector('#featureList');if(list)list.innerHTML='';return}ensureRoomSearch(box);const pos=loadHero(node),label=document.querySelector('#heroPosition');if(label)label.textContent=`${node.name} · Feld ${pos.x+1}/${grid.w} · ${pos.y+1}/${grid.h}`;renderFeatures(node,pos)}
