...
function render(){
  ...
  for(const a of assignments){
    ...
    const definition=definitionFor(a),item=document.createElement('div');item.className=`gm-content-item type-${a.type} state-${a.state}${locked?' payload-locked':''}`;
    ...
    const meta=document.createElement('small');meta.textContent=`${a.hidden?'verborgen · ':''}${a.source==='fixed'?'authored':`Pool ${a.source}`} · ${a.state} · ${anchorLabel(a.anchor)}${locked?` · hinter ${parent?.label||a.lockedBy} verborgen`:''}`;item.append(meta);
