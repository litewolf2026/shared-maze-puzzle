-- Content plan v4 adds reveals/lockedBy dependency metadata to materialized assignments.
-- Player-token updates may backfill these fields when absent, just like anchor/origin/
-- description/mechanics, but may never alter them once established.
...
begin
  ...
  loop
    ...
    if (v_new-'state'-'anchor'-'origin'-'description'-'mechanics'-'reveals'-'lockedBy') is distinct from
       (v_old-'state'-'anchor'-'origin'-'description'-'mechanics'-'reveals'-'lockedBy') then return false; end if;
    ...
    if v_old ? 'reveals' and v_new->'reveals' is distinct from v_old->'reveals' then return false; end if;
    if v_old ? 'lockedBy' and v_new->'lockedBy' is distinct from v_old->'lockedBy' then return false; end if;
