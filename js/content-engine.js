...
export function expandSlotConfig({map,slotConfig,profiles={profiles:{}},derivedByNode={}}){
  profiles=mergeReusableProfiles(profiles);
  const rooms={},rules=[...arr(slotConfig.rules),...(slotConfig.generation?.useReusableCore===false?[]:reusableRules())];
  for(const node of map.nodes){
    const context=contentContext(map,node,derivedByNode[node.id]||{}),slots=[];
    for(const rule of rules){if(!conditionMatches(rule.when||{},context))continue;for(const profileId of arr(rule.profiles||[rule.profile]).filter(Boolean)){const profile=profiles.profiles?.[profileId];if(!profile)throw new Error(`Unknown content profile ${profileId}`);mergeSlots(slots,profile.slots,{origin:'profile'})}}
    const authored=slotConfig.rooms?.[node.id];
    if(authored){for(const profileId of arr(authored.profiles||[authored.profile]).filter(Boolean)){const profile=profiles.profiles?.[profileId];if(!profile)throw new Error(`Unknown content profile ${profileId}`);mergeSlots(slots,profile.slots,{origin:'authored_profile'})}mergeSlots(slots,authored.slots,{replace:true,origin:'authored'})}
    const scenarioRoom=scenarioRoomConfig(slotConfig.scenario,node.id);if(scenarioRoom)mergeSlots(slots,scenarioRoom.slots,{replace:true,origin:'scenario'});
...
function materialize(item,slot,node,source){return {slotId:slot.id,contentId:item.id,type:item.type,label:item.label,source,origin:slot._origin||'authored',placement:slot.placement||item.placement?.features||[],hidden:Boolean(slot.hidden??item.hidden),discoverDifficulty:Number(slot.discoverDifficulty??item.discover?.difficulty??0),description:item.description||'',mechanics:item.mechanics?structuredClone(item.mechanics):null,reveals:arr(slot.reveals),lockedBy:slot.lockedBy||null,additiveOnUpgrade:Boolean(slot.additiveOnUpgrade),state:'unresolved',nodeId:node.id}}
...
function gridForNode(node){if(!node)return null;if(node.exploreGrid?.w&&node.exploreGrid?.h)return node.exploreGrid;if(!EXPLORE_KINDS.has(node.kind))return null;if(node.kind==='goal')return {w:7,h:7};if(node.kind==='prison')return {w:4,h:4};return {w:5,h:5}}
...
function validatePayloadLinks(assignments,nodeId){const bySlot=new Map(assignments.map(a=>[a.slotId,a]));for(const a of assignments){for(const targetId of arr(a.reveals)){const target=bySlot.get(targetId);if(!target)throw new Error(`${nodeId}/${a.slotId} reveals missing payload ${targetId}`);if(target.lockedBy!==a.slotId)throw new Error(`${nodeId}/${targetId} must declare lockedBy ${a.slotId}`)}if(a.lockedBy){const parent=bySlot.get(a.lockedBy);if(!parent)throw new Error(`${nodeId}/${a.slotId} locked by missing ${a.lockedBy}`);if(!arr(parent.reveals).includes(a.slotId))throw new Error(`${nodeId}/${a.slotId} lock is not reciprocally declared`)}}
}
...
export function materializeRoomState(state,plan,nodeId){
  ...
  if(current?.content?.generated){
    let changed=false;const plannedAssignments=planned.assignments||[],bySlot=new Map(plannedAssignments.map(a=>[a.slotId,a])),existing=new Set();
    for(const a of current.content.assignments||[]){existing.add(a.slotId);const p=bySlot.get(a.slotId);if(!a.anchor&&p?.anchor){a.anchor=structuredClone(p.anchor);changed=true}if(!a.origin&&p?.origin){a.origin=p.origin;changed=true}if(!a.description&&p?.description){a.description=p.description;changed=true}if(!a.mechanics&&p?.mechanics){a.mechanics=structuredClone(p.mechanics);changed=true}if(!Array.isArray(a.reveals)&&p?.reveals){a.reveals=structuredClone(p.reveals);changed=true}if(!a.lockedBy&&p?.lockedBy){a.lockedBy=p.lockedBy;changed=true}}
    for(const p of plannedAssignments){if(!existing.has(p.slotId)&&p.additiveOnUpgrade===true){current.content.assignments.push(structuredClone(p));changed=true}}
    if(Number(current.content.planVersion||0)<Number(planned.planVersion||0)){current.content.planVersion=planned.planVersion;changed=true}
...
export function contentDependencySatisfied(state,nodeId,assignment){if(!assignment?.lockedBy)return true;const parent=state?.roomState?.[nodeId]?.content?.assignments?.find?.(a=>a.slotId===assignment.lockedBy);return Boolean(parent&&OPEN_STATES.has(parent.state))}
export function contentVisibleToPlayer(assignment,state=null,nodeId=null){if(!assignment||TERMINAL_STATES.has(assignment.state))return false;if(assignment.lockedBy&&(!state||!nodeId||!contentDependencySatisfied(state,nodeId,assignment)))return false;if(assignment.hidden&&assignment.state==='unresolved')return false;if((TRIGGER_TYPES.has(assignment.type)||assignment.type==='secret'||assignment.type==='secret_connection')&&assignment.state==='unresolved')return false;return true}
...
export function applyContentAction(state,nodeId,slotId,action,{isGm=false}={}){
  ...
  else if(action==='take'){if(a.type!=='loot'||!['discovered','triggered'].includes(a.state))return {ok:false,state:next,error:'CONTENT_ACTION_FORBIDDEN'};target='taken'}
