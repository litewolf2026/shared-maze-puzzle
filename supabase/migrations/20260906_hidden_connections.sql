...
-- Player-token updates may materialize a referenced secret assignment for the first
-- time, but only in its initial unresolved state. Any later state change of that
-- unlock assignment is GM-authoritative. This prevents a client from submitting
-- "opened" and traversing the hidden edge in the same forged update.
create or replace function public.maze_player_unlock_states_unchanged(
  p_scenario_id text,
  p_old_state jsonb,
  p_new_state jsonb
)
...
begin
  for r in
    select distinct unlock_node_id,unlock_slot_id
    from public.maze_edge_requirements
    where scenario_id=p_scenario_id
  loop
    select a->>'state' into v_old
    from jsonb_array_elements(
      coalesce(p_old_state #> array['roomState',r.unlock_node_id,'content','assignments'],'[]'::jsonb)
    ) a
    where a->>'slotId'=r.unlock_slot_id
    limit 1;

    select a->>'state' into v_new
    from jsonb_array_elements(
      coalesce(p_new_state #> array['roomState',r.unlock_node_id,'content','assignments'],'[]'::jsonb)
    ) a
    where a->>'slotId'=r.unlock_slot_id
    limit 1;

    if v_old is null then
      if v_new is not null and v_new<>'unresolved' then return false; end if;
...
$$;
revoke execute on function public.maze_player_unlock_states_unchanged(text,jsonb,jsonb) from public,anon,authenticated;
