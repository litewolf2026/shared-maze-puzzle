...
function drawDoor(svg,frame,label=''){const w=Math.max(34,(frame.r-frame.l)*.28),h=Math.max(48,(frame.b-frame.t)*.62),x=(frame.l+frame.r-w)/2,y=frame.b-h;svg.append(el('rect',{x,y,width:w,height:h,rx:4,class:'crawler-room-door'}));if(label){const t=el('text',{x:x+w/2,y:y-7,class:'crawler-room-door-label'});t.textContent=label;svg.append(t)}}
function renderRoomScene(svg,node,exits,state){
  ...
  const features=state.visibleFeatures||[];features.slice(0,3).forEach((f,i)=>{const g=el('g',{class:'crawler-feature-marker'}),x=400+(i-(features.length-1)/2)*155,y=345-i%2*28;g.append(el('circle',{cx:x,cy:y,r:7,class:'crawler-feature-dot'}));const t=el('text',{x:x+12,y:y+4,class:'crawler-feature-text'});t.textContent=f.label;g.append(t);svg.append(g)});
...
function renderScene(svg,map,dirs,state){
  ...
  svg.dataset.level=String(current.z);svg.innerHTML='<defs><filter id="crawlerGlow"><feGaussianBlur stdDeviation="5" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter><pattern id="crawlerDust" width="28" height="28" patternUnits="userSpaceOnUse"><circle cx="5" cy="8" r="1" class="crawler-dust-dot"/><circle cx="21" cy="19" r="1.4" class="crawler-dust-dot"/><path d="M2 24l9-3M17 5l7 2" class="crawler-dust-line"/></pattern></defs>';
  svg.append(el('rect',{x:0,y:0,width:800,height:450,class:'crawler-bg'}));svg.append(el('rect',{x:0,y:0,width:800,height:450,fill:'url(#crawlerDust)',class:'crawler-dust'}));
