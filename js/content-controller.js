...
function ensureStyles(){if(document.querySelector('#gmContentStyle'))return;const s=document.createElement('style');s.id='gmContentStyle';s.textContent='.gm-content{margin-top:12px;padding-top:10px;border-top:1px solid #4b3d2d;max-height:280px;overflow:auto}.gm-content h4{margin:0 0 8px;font-size:13px}.gm-content>small{color:#9f9079}.gm-content-item{padding:7px;margin-top:6px;border:1px solid #493b2c;border-radius:8px;background:#120f0c}.gm-content-head{display:flex;gap:7px;align-items:baseline}.gm-content-head span{font-size:9px;text-transform:uppercase;letter-spacing:.1em;color:#b29567}.gm-content-head b{font-size:11px}.gm-content-item>small{display:block;margin-top:4px;color:#8f816d;font-size:9px}.gm-content-item.state-resolved,.gm-content-item.state-taken{opacity:.5}';document.head.append(s)}
...
function render(){
  ...
  for(const a of assignments){
    ...
    const meta=document.createElement('small');meta.textContent=`${a.hidden?'verborgen · ':''}${a.source==='fixed'?'authored':`Pool ${a.source}`} · ${a.state}`;item.append(meta);box.append(item);
  }
}
